...
Whenever a sign-in attempt is completed from an unfamiliar device, Clerk sends an email notification to the account owner with information about the newly created session. The email varies depending on the instance's configuration and the application's billing plan.
...
1. Select **Sign in from new client**. You'll be redirected to the template settings page.
...
> [!WARNING]
> This feature isn't available in production for free plans. However, you can try it out for free in development. For more information, see the [pricing](/pricing){{ target: '_blank' }} page.

For supported instances, the email might also include a button allowing users to immediately revoke unauthorized sign-ins. Clicking this button revokes the suspicious session and redirects the user to a confirmation page. The contents of the confirmation page depend on the instance's configuration.
